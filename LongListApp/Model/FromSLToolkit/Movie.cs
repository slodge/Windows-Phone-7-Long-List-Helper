...
namespace PhoneToolkitSample.Data
{
    public class Movie
    {
        ...
        // small mod changes in this method from the original version
        public static Movie CreateRandom(string category)
        {
            Movie movie = new Movie();
            movie.Title = LoremIpsum.GetWords(_rnd.Next(1, 3), LoremIpsum.Capitalization.AllWords);
            movie.Year = _rnd.Next(1965, 2010);
            movie.Rating = _rnd.Next(Ratings);
            movie.RunTime = TimeSpan.FromMinutes(_rnd.Next(60, 180));
            movie.Description = LoremIpsum.GetParagraph(_rnd.Next(1, 3)); 
            movie.Star1 = RandomPeople.GetRandomFullName(5);
            movie.Star2 = RandomPeople.GetRandomFullName(5);
            movie.Category = category;
            movie.ImageUrl = "/Images/" + category + ".jpg";
